...
Паттерн отпила сикросервиса от монолита. В отличие от [[Strangler fig pattern]] подходит для случаев, когда вызовы к отпиливаемому функционалу происходят внутри монолита.
...
Монолитная реализация может выполнять функции fallback'a в случае отказа микросервиса. Развитием решения может быть [[Parallel run pattern]]: для валидации работы микросервиса можно сравнивать его ответ с ответом монолитной реализации.
